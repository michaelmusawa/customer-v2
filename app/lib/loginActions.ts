...
export async function getUser(email: string): Promise<User | undefined> {
  try {
    const sql = `
      ...
      WHERE u.email = @email`;
    const { recordset } = await safeQuery<User>(sql, { email });
    return recordset[0];
  } catch (error) {
...
export async function forgetPassword(
  ...
  try {
    const sqlLookup = `SELECT id FROM [User] WHERE email = @email`;
    const { recordset } = await safeQuery<{ id: number }>(sqlLookup, { email });
    ...
    const sqlStore = `
      ...
      WHERE id = @id`;
    await safeQuery(sqlStore, { token, expires, id: recordset[0].id });
...
export default async function resetPasswordHandler(
  ...
  const sqlLookup = `
    SELECT id, password_reset_expires AS expires
    FROM [User]
    WHERE password_reset_token = @token`;
  const { recordset } = await safeQuery<{ id: number; expires: Date }>(
    sqlLookup,
    { token }
  );
  ...
  const sqlUpdate = `
    ...
    WHERE id = @id`;
  await safeQuery(sqlUpdate, { hash, id: user.id });
